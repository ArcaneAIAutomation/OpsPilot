...
export class DashboardModule implements IModule {
  ...
  private detectEnvironmentVars(): EnvVarInfo[] {
    const defs: Array<{ name: string; purpose: string; category: string }> = [
      ...
      { name: 'OPSPILOT_EMAIL_CLIENT_ID', purpose: 'OAuth Client ID for email notifications', category: 'notifier' },
      { name: 'OPSPILOT_EMAIL_CLIENT_SECRET', purpose: 'OAuth Client Secret for email notifications', category: 'notifier' },
      { name: 'OPSPILOT_EMAIL_REFRESH_TOKEN', purpose: 'OAuth Refresh Token for email notifications', category: 'notifier' },
      { name: 'SMTP_HOST', purpose: 'SMTP server for email notifications (legacy)', category: 'notifier' },
      { name: 'SMTP_PASSWORD', purpose: 'SMTP password for email notifications (legacy)', category: 'notifier' },
